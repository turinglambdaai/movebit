...
- Added switchable 7-day and 30-day activity-history views while retaining up to 370 days of persisted history
- Added work-pattern insights for the current continuous session, today's longest continuous session, recent longest session, active-day average, busiest recent day, and sedentary-streak guidance
- Added backward-compatible persistence of each day's longest continuous-work session
- Added Linux/Wayland idle detection through GNOME/Mutter `org.gnome.Mutter.IdleMonitor` and the freedesktop ScreenSaver session-bus API, with a trustworthy elapsed-time fallback when neither API is available
- Added native macOS arm64 `MoveBit.app`, `.app.zip`, and DMG packaging with generated application metadata/icon and SHA-256 sidecars
- Added Debian/Ubuntu amd64 `.deb` packaging with `/opt/movebit`, `/usr/bin/movebit`, desktop-menu integration, application icon, and SHA-256 sidecar
- Added CI smoke tests that mount the generated DMG and verify its app bundle, and install/remove the generated Debian package
- Extended release automation to publish the new macOS native packages and Linux `.deb` alongside the existing portable ZIPs and Windows installer

### Changed

- Promoted MoveBit to 1.1.0 for the completed non-signing roadmap milestone
- Native/system-owned installations document package-manager/DMG upgrades while portable and user-writable installs continue using MoveBit's verified ZIP updater
- The main window now keeps the native window bounds unconstrained while maximized, preventing title-bar and Avalonia client-area width mismatches
- English and Chinese documentation now describe current Wayland behavior, native packages, work-pattern insights, and the remaining paid signing/notarization item accurately

### Fixed

- Maximized windows could display native title-bar chrome wider than the Avalonia client content because normal-window sizing constraints could leak into maximized state
- Wayland D-Bus output parsing now ignores numeric type-name suffixes such as `uint64`/`uint32` instead of mistaking them for the idle value

[1.1.0]: https://github.com/turinglambdaai/movebit/releases/tag/v1.1.0

## [1.0.5] - 2026-09-22

### Added

- Added a configurable 5–60 minute “remind me later” delay, defaulting to 10 minutes and shared by reminder toasts and skipped forced breaks
- Reminder and forced-break buttons now show the configured delay directly in their labels

### Changed

- Bumped MoveBit to 1.0.5
- Small high-DPI displays can reduce the main window's minimum height so all settings remain reachable through vertical scrolling

### Fixed

- Maximizing the main window no longer leaves the native title bar wider than the Avalonia content area

[1.0.5]: https://github.com/turinglambdaai/movebit/releases/tag/v1.0.5
...
- Single instance enforcement: launching a second copy now exits immediately instead of stacking a second tray icon, a second scheduler (double reminders, double break overlays) and a second history writer that clobbers the first one's data. The second launch pokes the running instance over a loopback channel to surface its window — re-launching feels like "bring it up", not nothing. The break lock suppresses the poke: no window fights the overlay.
...
- Onboarding choice cards showed no selection state at all: the style selector used `Border.choice:selected` (pseudo-class syntax) instead of `Border.choice.selected` (class syntax), so the selected card never highlighted and the choices looked unclickable — clicks did register, the feedback just never rendered
...
- Micro-break interval/duration number boxes were too narrow (95 DIP) to show two-digit values — widened to match the other settings rows
- Week card header: "本周活跃 …" total overlapped the "最近 7 天" label (missing `Grid.Column` assignment); now sits on the right as intended
...
- **Activity history**: daily stats persist to `history.json` (flushed every ~5 min, archived at midnight) and render as a last-7-days bar chart in the main window — the "monitor my work time" promise now survives restarts
- **Micro breaks**: third reminder layer — a screen-center "stand up, look far away" nudge every 30 min (default 20 s, no lock, no sound, Esc/click to dismiss); independent cycle that resets on away, never blocks the long-break cycle
- **Autostart on login**: one toggle in settings (Windows HKCU Run key / macOS LaunchAgent / Linux XDG autostart)
- **Light/dark theme** following the system: paper-warm palette for the day, warm-dark for night; all windows migrated to theme resources
...
- Main window: today card gains micro-break count, new micro-break settings block, autostart toggle; window grows to fit the week chart
...
- Real four-step onboarding replaces the welcome card: meet the droplet → pick a strictness pact (gentle toast-only / standard / strict-research-backed, the 30-min tier is labeled as closest to the evidence) → water cadence and sound → summary of the pact with live values
- Choices write through to the config immediately; skipping applies clean defaults; closing the window counts as seen so onboarding never nags on every launch

### Changed

- Welcome is seen-once: dismissing the card or closing the window marks it read; it never reappears on later launches
...
- First-run welcome: on a fresh install the main window opens with a greeting card from the droplet that sets expectations (screen-lock behavior, skip delay, away reset, close-to-tray) with the live settings values baked in — a tool that locks every screen in 45 minutes owes the user an explanation first
...
- Forced break lock is now silent: the screen takeover is its own notification — a beep plus a full-screen lock is exactly the office embarrassment that gets health tools uninstalled; toast reminders still honor the sound toggle
...
- Fun copy pools: randomized break hints (rotated every 25 s during the break) and randomized water/sit toast lines — same-message fatigue is the enemy of attention
...
- Visual redesign: warm paper palette for the main window (cream canvas, ink text, terracotta accents), night-sky gradient break screen with a breathing countdown
...
- Sit reminder cycle (default 45 min) with full-screen multi-monitor forced break lock: countdown, Alt+F4-proof, delayed skip button (default 20 s)
